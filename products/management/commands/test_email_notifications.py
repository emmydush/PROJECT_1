...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument('--email', type=str, help='Email address to send test notifications to')
    ...
    def handle(self, *args, **options):
        ...
        if not email_address:
            ...
            if admin_user and admin_user.email:
                ...
                self.stdout.write(
                    self.style.ERROR('No email address provided and no admin user with email found.')
                )
                return
        ...
        try:
            ...
            self.stdout.write(
                self.style.SUCCESS(f'Successfully sent low stock test email to {email_address}')
            )
        except Exception as e:
            self.stdout.write(
