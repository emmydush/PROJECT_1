...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument('--email', type=str, help='Email of the user to create products for', required=True)
        parser.add_argument('--count', type=int, help='Number of products to create', default=100)
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(f'Creating {count} products for user {user.username} with business {business.company_name}')
        ...
        product_prefixes = [
            'Premium', 'Advanced', 'Professional', 'Deluxe', 'Standard', 
            'Basic', 'Economy', 'Luxury', 'Classic', 'Modern'
        ]
        
        product_names = [
            'Smartphone', 'Laptop', 'Tablet', 'Headphones', 'Speaker',
            'Camera', 'Watch', 'Fitness Tracker', 'Gaming Console', 'TV',
            'T-Shirt', 'Jeans', 'Jacket', 'Dress', 'Shoes',
            'Coffee', 'Tea', 'Juice', 'Soda', 'Water',
            'Tool Set', 'Hammer', 'Screwdriver', 'Wrench', 'Drill',
            'Shampoo', 'Soap', 'Lotion', 'Perfume', 'Makeup',
            'Football', 'Basketball', 'Tennis Racket', 'Golf Club', 'Yoga Mat',
            'Novel', 'Textbook', 'Magazine', 'Comic', 'Journal',
            'Toy Car', 'Doll', 'Puzzle', 'Board Game', 'Action Figure'
        ]
        
        product_suffixes = [
            'Pro', 'Plus', 'Max', 'Mini', 'Lite',
            'XL', 'XXL', 'Ultra', 'Smart', 'Digital'
        ]
        ...
        for i in range(count):
            ...
            selling_price = Decimal(str(round(float(cost_price) * random.uniform(1.1, 3.0), 2)))
