...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if not admin_emails:
            self.stdout.write(
                self.style.WARNING('No admin users with email addresses found. Skipping email notifications.')
            )
            return
        ...
        if expired_products:
            self.send_expired_products_email(admin_emails, expired_products, business_name)
        ...
        if near_expiry_products:
            self.send_near_expiry_email(admin_emails, near_expiry_products, business_name, expiry_alert_days)
        ...
        self.stdout.write(
            self.style.SUCCESS(f'Successfully sent expiry email notifications to {len(admin_emails)} recipients')
        )
    ...
    def send_expired_products_email(self, admin_emails, expired_products, business_name):
        ...
        try:
            ...
            self.stdout.write(
                self.style.SUCCESS(f'Sent expired products email to {len(admin_emails)} recipients')
            )
    ...
    def send_near_expiry_email(self, admin_emails, near_expiry_products, business_name, days_threshold):
        ...
        try:
            ...
            self.stdout.write(
                self.style.SUCCESS(f'Sent near expiry products email to {len(admin_emails)} recipients')
            )
        except Exception as e:
            self.stdout.write(
    ...
    def send_low_stock_email(self, admin_emails, low_stock_products, business_name):
        ...
        try:
            ...
            self.stdout.write(
                self.style.SUCCESS(f'Sent low stock products email to {len(admin_emails)} recipients')
            )
        except Exception as e:
            self.stdout.write(
