...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        users_with_businesses = User.objects.filter(owned_businesses__isnull=False).distinct()
        
        self.stdout.write(f"Found {users_with_businesses.count()} users with businesses")
        
        for user in users_with_businesses:
            user_businesses = user.owned_businesses.all()
            self.stdout.write(f"\nUser: {user.username}")
            self.stdout.write(f"Businesses: {[b.company_name for b in user_businesses]}")
            ...
            self.stdout.write(f"Business-specific notifications: {business_notifications.count()}")
            
            # Check if all notifications belong to user's businesses
            all_correct = True
            for notification in user_notifications:
                if notification.business and notification.business not in user_businesses:
                    self.stdout.write(
                        self.style.ERROR(
