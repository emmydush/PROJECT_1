    ...
    def handle(self, *args, **options):
        try:
            ...
            if barcode_buffer:
                ...
                self.stdout.write(
                    self.style.SUCCESS(f'Successfully generated test barcode: {barcode_path}')
                )
                self.stdout.write(
                ...
                self.stdout.write(
                    self.style.ERROR('Failed to generate barcode image')
                )
            ...
            self.stdout.write(
                self.style.ERROR(f'Error generating barcode: {str(e)}')
            )