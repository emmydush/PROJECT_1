...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument('--email', type=str, help='Email of the user to reduce sales for', required=True)
        parser.add_argument('--keep', type=int, help='Number of sales to keep', default=20)
    ...
    def handle(self, *args, **options):
        ...
        if total_sales <= keep_count:
            self.stdout.write(f'User already has {total_sales} sales, which is less than or equal to {keep_count}. No reduction needed.')
            return
