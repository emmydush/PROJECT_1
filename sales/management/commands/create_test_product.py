...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if created:
            self.stdout.write(
                self.style.SUCCESS(f'Successfully created test product: {product.name} with barcode {product.barcode}')
            )
        else:
            self.stdout.write(
                self.style.SUCCESS(f'Test product already exists: {product.name} with barcode {product.barcode}')
            )