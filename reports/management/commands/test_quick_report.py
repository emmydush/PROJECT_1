...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(f'Testing quick report view for business: {business.company_name}')
        ...
        for period in periods:
            ...
            try:
                ...
                if response.status_code == 200:
                    self.stdout.write(self.style.SUCCESS(f'  ✓ {period.capitalize()} quick report: OK'))
                else:
                    self.stdout.write(self.style.ERROR(f'  ✗ {period.capitalize()} quick report: FAILED - Status {response.status_code}'))
            except Exception as e:
                self.stdout.write(self.style.ERROR(f'  ✗ {period.capitalize()} quick report: FAILED - {str(e)}'))
