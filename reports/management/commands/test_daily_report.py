...
from reports.management.commands.send_daily_report import Command as SendDailyReportCommand
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            for report_data in all_reports_data:
                ...
                self.stdout.write(f"Report Period: {report_data['start_date']} to {report_data['end_date']}")
                ...
                self.stdout.write(f"  Total Expenses: ${report_data['total_expenses']:.2f}")
                ...
                for product in report_data['top_products']:
                    self.stdout.write(f"  - {product['product__name']}: {product['total_sold']} units (${product['total_revenue']:.2f})")
                self.stdout.write('-' * 40)
            ...
            self.stdout.write(self.style.SUCCESS('Daily reports generation test for all accounts completed successfully!'))
            
        except Exception as e:
            self.stdout.write(
                self.style.ERROR(f'Failed to generate daily reports for all accounts: {str(e)}')
            )