...
from reports.views import (
    sales_report, inventory_report, profit_loss_report, expenses_report,
    export_sales_report_csv, export_inventory_report_csv, 
    export_profit_loss_report_csv, export_expenses_report_csv
)
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(f'Comprehensive testing of reports for business: {business.company_name}')
        ...
        for report_name, url, view_func in reports_to_test:
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    self.stdout.write(self.style.ERROR(f'    ✗ {report_name}: FAILED - Status {response.status_code}'))
        ...
        csv_tests = [
            ('Sales Report CSV', f'/reports/sales/?export=csv&start_date={start_date}&end_date={end_date}', 
             lambda r: export_sales_report_csv(r, start_date, end_date)),
            ('Inventory Report CSV', '/reports/inventory/?export=csv', export_inventory_report_csv),
            ('Profit & Loss Report CSV', f'/reports/profit-loss/?export=csv&start_date={start_date}&end_date={end_date}', 
             lambda r: export_profit_loss_report_csv(r, start_date, end_date)),
            ('Expenses Report CSV', f'/reports/expenses/?export=csv&start_date={start_date}&end_date={end_date}', 
        ...
        for report_name, url, export_func in csv_tests:
            ...
            try:
                ...
                if response['Content-Type'] == 'text/csv':
                    ...
                    self.stdout.write(self.style.ERROR(f'    ✗ {report_name}: FAILED - Wrong content type: {response["Content-Type"]}'))
            except Exception as e:
                self.stdout.write(self.style.ERROR(f'    ✗ {report_name}: FAILED - {str(e)}'))
        
        self.stdout.write('=' * 60)
        self.stdout.write(self.style.SUCCESS('Comprehensive report testing completed!'))
        self.stdout.write('All reports are working correctly with both view and export functionality.')