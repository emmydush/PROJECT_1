...
from reports.views import sales_report, inventory_report, profit_loss_report, expenses_report
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(f'Testing print functionality for reports in business: {business.company_name}')
        self.stdout.write('=' * 60)
        
        # Test each report template for print functionality
        reports_to_test = [
            ('Sales Report', '/reports/sales/', sales_report, 'reports/sales.html'),
            ('Inventory Report', '/reports/inventory/', inventory_report, 'reports/inventory.html'),
            ('Profit & Loss Report', '/reports/profit-loss/', profit_loss_report, 'reports/profit_loss.html'),
            ('Expenses Report', '/reports/expenses/', expenses_report, 'reports/expenses.html'),
        ]
        ...
        for report_name, url, view_func, template_name in reports_to_test:
            ...
            try:
                ...
                if 'window.print()' in content or 'onclick="window.print()"' in content:
                    self.stdout.write(self.style.SUCCESS(f'  ✓ {report_name}: Print button found'))
                else:
                    self.stdout.write(self.style.ERROR(f'  ✗ {report_name}: Print button NOT found'))
                
                # Check if response contains the export CSV button
                if 'export=csv' in content:
                    self.stdout.write(self.style.SUCCESS(f'  ✓ {report_name}: Export CSV button found'))
                else:
                    self.stdout.write(self.style.ERROR(f'  ✗ {report_name}: Export CSV button NOT found'))
