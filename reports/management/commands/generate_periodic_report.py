...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(f'Generating {period} report for {report_date} for business: {business.company_name}')
        ...
        for report_name, url, view_func in reports_to_test:
            ...
            try:
                request = factory.get(f'{url}?start_date={start_date}&end_date={end_date}')
                request.user = user
                response = view_func(request)
                if response.status_code == 200:
                    self.stdout.write(self.style.SUCCESS(f'    ✓ {report_name}: Generated successfully'))
                else:
        ...
        self.stdout.write('=' * 60)
        self.stdout.write(self.style.SUCCESS(f'{period.capitalize()} report generation completed for {period_name}!'))