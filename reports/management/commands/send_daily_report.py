...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        is_scheduled_time = current_time.hour == 23 and current_time.minute == 0  # 11 PM
    ...
    def generate_all_business_reports(self):
        ...
        for business in businesses:
            try:
                ...
                top_products = SaleItem.objects.filter(sale__business=business).filter(
                ...
                logger.error(f'Failed to generate report for business {business.company_name}: {str(e)}')
                self.stdout.write(
                    self.style.WARNING(f'Failed to generate report for business {business.company_name}: {str(e)}')
                )
    ...
    def create_csv_attachment(self, all_reports_data):
        ...
        writer.writerow(['Report Date:', all_reports_data[0]['start_date'] if all_reports_data else 'N/A'])
        writer.writerow([])
    ...
    def send_report_email(self, email_address, all_reports_data):
        ...
        html_content = render_to_string('reports/email_all_daily_reports.html', {
            'reports': all_reports_data,
            'report_date': all_reports_data[0]['start_date'] if all_reports_data else 'N/A',
        })
...
"""
            ...
            text_content += "\nRecommendations:\n"
            for rec in report_data['recommendations']:
                text_content += f"- {rec.get('title', '')}: {rec.get('description', '')}\n"
            text_content += "\n" + "="*50 + "\n"
        ...
        email.attach(f"daily_reports_all_accounts_{all_reports_data[0]['start_date'] if all_reports_data else 'N/A'}.csv", csv_content, 'text/csv')
