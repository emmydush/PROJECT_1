...
def admin_required(view_func):
    ...
    def _wrapped_view(request, *args, **kwargs):
        ...
        if request.user.role != 'admin':
            messages.error(request, 'You do not have permission to access this page. Only administrators can manage users.')
...
def barcode_settings(request):
    ...
    return render(request, 'settings/barcode.html', {'form': form, 'settings': barcode_settings})
...
def email_settings(request):
    ...
    return render(request, 'settings/email.html', {'form': form, 'settings': email_settings})
                ...
                with zipfile.ZipFile(backup_path, 'w') as backup_zip:
                    ...
                    try:
                        ...
                        if os.path.exists(temp_filename) and os.path.getsize(temp_filename) > 0:
                            ...
                            print(f"Added PostgreSQL database dump using Django dumpdata")
            ...
            try:
                ...
                with zipfile.ZipFile(temp_filename, 'r') as backup_zip:
                    ...
                    if database_dump:
                        ...
                        with tempfile.NamedTemporaryFile(suffix='.json', delete=False) as db_temp_file:
                            db_temp_filename = db_temp_file.name
                        
                        # Extract database dump to temporary file
                        with backup_zip.open(database_dump) as source, open(db_temp_filename, 'wb') as target:
                            target.write(source.read())
                        
                        # Use Django's loaddata command to restore the database
                        try:
                            management.call_command('loaddata', db_temp_filename, verbosity=0)
                        ...
                        messages.warning(request, 'No database dump found in backup file.')
                    ...
                    for filename in backup_zip.namelist():
                        if filename.startswith('media/'):
                            # Extract media file
                            extracted_path = backup_zip.extract(filename, tempfile.gettempdir())
                            # Move to correct location
                            target_path = os.path.join(media_root, filename[6:])  # Remove 'media/' prefix
                            os.makedirs(os.path.dirname(target_path), exist_ok=True)
                            os.replace(extracted_path, target_path)
                            media_files_restored += 1
                    
                    if media_files_restored > 0:
                        messages.success(request, f'Restored {media_files_restored} media files.')
